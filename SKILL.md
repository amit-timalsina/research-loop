...
**Caveat: smoke tests must use a slice of the real data, not synthetic random.** Observed Sprint 5 W5_3: a custom IQL trainer passed a smoke test on 100 random-Gaussian data points but NaN'd within 2000 steps on the actual CCR2 industrial dataset. The cause: real reward magnitudes (mean ≈ 67) made the early Bellman target O(6700), and a joint optimizer over a shared encoder + heads couldn't survive the resulting gradient explosion. Synthetic Gaussian data has reward ~ N(0,1), which doesn't stress this. **Going forward, smoke tests for trainers should use a 1k-row slice of the real benchmark data with the real reward formula, run ≥500 train steps + 1 FQE eval, and assert `not np.isnan(loss)` at every checkpoint.** A trainer that passes synthetic-only smoke and NaNs on real data has not been validated, and a research session that locks in such a trainer wastes a sprint cycle producing all-NaN results.

**Caveat: eval-protocol decisively matters.** Observed three times within a single project across sprints 6-8: (1) Sprint 6 W6_6 found CityLearn's PH3 verdict depends on initial-state distribution — single-buffer-state V gives "Fail-non-discrimination," 10-source-first-rows V gives clean BC ≫ const separation; (2) Sprint 7 W7_1 found cross-reward FQE comparisons inflate ratios when denominators are near-zero (a 30% real lift presented as 409%); (3) Sprint 8 W8_1 found CL's BC-vs-IQL ranking *flips* between FQE-V (BC > IQL) and gym-env per_step (IQL > BC) — same data, opposite verdicts. **When both an OPE metric and a real-environment metric are available, prefer the real-environment metric for confirmatory verdicts.** OPE adds an extra layer of noise on top of the underlying policy comparison, and ratio-based decision rules with small denominators are particularly fragile. When you must use OPE, document which protocol was chosen, sanity-check against alternative protocols (different initial-state distributions, different aggregators), and report the verdict envelope rather than a point estimate. A confirmatory verdict that flips based on protocol choice is not actually confirmatory.
